...
async def get_banners(
    query: str | None = Query(None, description="조회할 타입 (banner/promotion)", regex="^(banner|promotion)$")
):
    """배너 목록 조회"""
...
async def create_banner(
    ...
    """배너 생성"""
    request = BannerCreateRequest(title=title, redirect_url=redirect_url, banner_type=banner_type, is_active=is_active)
...
async def update_banner(
    ...
    request = BannerUpdateRequest(
        title=title, redirect_url=redirect_url, is_active=is_active, display_order=display_order
    )
